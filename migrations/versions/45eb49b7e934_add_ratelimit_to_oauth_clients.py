...
def upgrade():
    ...
    with op.batch_alter_table("users") as batch_op:
        batch_op.alter_column(column_name="ratelimit_level", type_=sa.types.Float)
        batch_op.alter_column(column_name="ratelimit_level", server_default="2.0")
    ...
    for user in connection.execute(users.select()):
        if user.email == "anonymous@ads":
            connection.execute(
                users.update().where(users.c.id == user.id).values(ratelimit_level=-1.0)
            )
            connection.execute(
                clients.update().where(clients.c.user_id == user.id).values(ratelimit=1.0)
            )
        elif user.email and user.email.endswith("@ads"):
            connection.execute(
                clients.update()
                .where(clients.c.user_id == user.id)
                .values(ratelimit=user.ratelimit_level)
            )
...
def downgrade():
    ...
    with op.batch_alter_table("users") as batch_op:
        batch_op.alter_column(column_name="ratelimit_level", type_=sa.types.Integer)
        batch_op.alter_column(column_name="ratelimit_level", server_default="2")
